...
<section class="flex row center space-between">
    <p>
        you are viewing a past version of this paste, <a href="/{data.paste.id}"
    ...
    <div class="flex row center gap-s">
        <a
            href={data.previousEdit ? `/${data.paste.id}/history/${data.previousEdit?.id}` : null}
            class="btn"
        ...
        <a
            href={data.nextEdit ? `/${data.paste.id}/history/${data.nextEdit?.id}` : null}
            class="btn"
