...
<section class="flex row center space-between danger">
    <p>you are viewing the diff of the paste, <a href="/{data.paste.id}">go back to the current version</a></p>

    <div class="flex row center gap-s">
	<a href={data.previousEdit ? `/${data.paste.id}/history/${data.previousEdit?.id}/diff` : null} class="btn" aria-label="previous edit" use:tooltip class:disabled={!data.previousEdit}>
	...
	<a href={data.nextEdit ? `/${data.paste.id}/history/${data.nextEdit?.id}/diff` : null} class="btn" aria-label="next edit" use:tooltip class:disabled={!data.nextEdit}>
	    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" class="icon">
		<path
		    fill="currentColor"
...
{#if data.oldTitle !== data.newTitle}
    <section>
	<p>title: <code class="deleted">{data.oldTitle}</code> to <code class="added">{data.newTitle}</code></p>
    </section>
{/if}
...
{#each data.modifiedPasties as { oldPasty, newPasty }}
    <DiffEditor oldPasty={oldPasty} newPasty={newPasty} settings={data.settings} />
{/each}
