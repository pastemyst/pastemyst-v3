import { getPasteDiff, getPasteHistoryCompact, getPasteLangs, getPasteStats, isPasteStarred, type Pasty } from "$lib/api/paste";
...
export const load: PageLoad = async ({ params, fetch, parent }) => {
    ...
    const addedPasties = pasteDiff.newPaste.pasties.filter(p => !pasteDiff.oldPaste.pasties.some(p2 => p2.id === p.id));
    const deletedPasties = pasteDiff.oldPaste.pasties.filter(p => !pasteDiff.newPaste.pasties.some(p2 => p2.id === p.id));
    ...
    return {
        paste, owner, pasteStats, langStats, isStarred, previousEdit, nextEdit, historyId: params.history,
        oldTitle, newTitle, addedPasties, deletedPasties, modifiedPasties
    };
};
