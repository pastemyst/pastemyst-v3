...
func (q *Queries) GetPasteStarCount(ctx context.Context, pasteID string) (int64, error) {
	row := q.db.QueryRowContext(ctx, getPasteStarCount, pasteID)
	var count int64
	err := row.Scan(&count)
	return count, err
}
...
func (q *Queries) IsPasteStarred(ctx context.Context, arg IsPasteStarredParams) (bool, error) {
	row := q.db.QueryRowContext(ctx, isPasteStarred, arg.UserID, arg.PasteID)
	var exists bool
	err := row.Scan(&exists)
	return exists, err
}
...
func (q *Queries) StarPaste(ctx context.Context, arg StarPasteParams) error {
	_, err := q.db.ExecContext(ctx, starPaste, arg.UserID, arg.PasteID)
	return err
}
...
func (q *Queries) UnstarPaste(ctx context.Context, arg UnstarPasteParams) error {
	_, err := q.db.ExecContext(ctx, unstarPaste, arg.UserID, arg.PasteID)
	return err
}
