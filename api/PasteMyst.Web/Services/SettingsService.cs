...
public class SettingsService(
    ...
    MongoService mongo)
{
    public async Task SetUsernameAsync(string username, CancellationToken token)
    {
        ...
        if (await userProvider.ExistsByUsernameAsync(username, token))
        {
        ...
        await mongo.Users.UpdateOneAsync(u => u.Id == userContext.Self.Id, update, cancellationToken: token);
    }

    public async Task SetAvatarAsync(byte[] bytes, string contentType, CancellationToken token)
    {
        if (!userContext.IsLoggedIn())
        {
            throw new HttpException(HttpStatusCode.Unauthorized, "You need to be authorized to change settings.");
        ...
        await mongo.Images.DeleteAsync(ObjectId.Parse(userContext.Self.AvatarId), token);
    ...
    public async Task UpdateUserSettingsAsync(UserSettings settings, CancellationToken token)
    {
        if (!userContext.IsLoggedIn())
    ...
    public async Task UpdateSettingsAsync(HttpContext httpContext, Settings settings, CancellationToken token)
    {
        if (userContext.IsLoggedIn())
        {
            if (!userContext.HasScope(Scope.User))
        ...
        else
        {
            ...
            var sessionSettings = await mongo.SessionSettings.Find(s => s.Id == sessionSettingsCookie).FirstOrDefaultAsync(cancellationToken: token);
            ...
            await mongo.SessionSettings.UpdateOneAsync(s => s.Id == sessionSettings.Id, update, cancellationToken: token);
        }
    }
