...
public class PasteService(
    ...
    MongoService mongo)
{
    public async Task<Paste> CreateAsync(ClaimsPrincipal self, PasteCreateInfo createInfo)
    {
        switch (createInfo.Pinned)
        {
            case true when !self.IsLoggedIn():
        ...
        var paste = new Paste
        {
            ...
            OwnerId = createInfo.Anonymous ? null : self.Id(),
    ...
    public async Task<Paste> GetAsync(ClaimsPrincipal self, string id)
    {
        ...
        if (paste.Private && (!self.IsLoggedIn() || self.Id() != paste.OwnerId))
            throw new HttpException(HttpStatusCode.NotFound, "Paste not found");

        // only the paste owner can see the tags
        if (!self.IsLoggedIn() || self.Id() != paste.OwnerId)
            paste.Tags = [];

        return paste;
    }

    public async Task<PasteStats> GetStatsAsync(ClaimsPrincipal self, string id)
    {
        var paste = await GetAsync(self, id);
    ...
    public async Task<List<LanguageStat>> GetLanguageStatsAsync(ClaimsPrincipal self, string id)
    {
        var paste = await GetAsync(self, id);
    ...
    public List<LanguageStat> GetLanguageStats(Paste paste)
    {
        ...
        stats.AddRange(from entry in charsPerLanguage
                        let percentage = entry.Value / (float)totalChars * 100 where percentage != 0
                        let language = languageProvider.FindByName(entry.Key)
                        select new LanguageStat { Language = language, Percentage = percentage });
    ...
    public async Task DeleteAsync(ClaimsPrincipal self, string id)
    {
        if (!self.IsLoggedIn())
        ...
        if (paste.OwnerId is null || paste.OwnerId != self.Id())
        {
    ...
    public async Task<bool> IsStarredAsync(ClaimsPrincipal self, string id)
    {
        if (!self.IsLoggedIn())
        ...
        if (paste.OwnerId is not null && paste.OwnerId == self.Id())
            return paste.Stars.Any(u => u == self.Id());
    ...
    public async Task ToggleStarAsync(ClaimsPrincipal self, string id)
    {
        if (!self.IsLoggedIn())
        {
            throw new HttpException(HttpStatusCode.Unauthorized, "You must be authorized to star pastes.");
        ...
        if (paste.Stars.Any(u => u == self.Id()))
        {
            paste.Stars.Remove(self.Id());
        }
        else
        {
            paste.Stars.Add(self.Id());
        }
    ...
    public async Task TogglePinnedAsync(ClaimsPrincipal self, string id)
    {
        if (!self.IsLoggedIn())
        {
    ...
    public async Task TogglePrivateAsync(ClaimsPrincipal self, string id)
    {
        if (!self.IsLoggedIn())
        {
    ...
    public async Task<(byte[] zip, string title)> GetPasteAsZip(ClaimsPrincipal self, string id)
    {
        var paste = await GetAsync(self, id);
    ...
    public async Task<Paste> EditTagsAsync(ClaimsPrincipal self, string id, List<string> tags)
    {
        if (!self.IsLoggedIn())
        {
    ...
    public async Task<Paste> EditAsync(ClaimsPrincipal self, string id, PasteEditInfo editInfo)
    {
        if (!self.IsLoggedIn())
        {
    ...
    public async Task<List<PasteHistoryCompact>> GetHistoryCompactAsync(ClaimsPrincipal self, string id)
    {
        var paste = await GetAsync(self, id);
    ...
    public async Task<Paste> GetAtEditAsync(ClaimsPrincipal self, string id, string historyId)
    {
        var paste = await GetAsync(self, id);
    ...
    public async Task<PasteDiff> GetDiffAsync(ClaimsPrincipal self, string id, string historyId)
    {
        var paste = await GetAsync(self, id);
